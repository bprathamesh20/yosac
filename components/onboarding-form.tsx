...
const steps = [
  {
    id: 1,
    name: 'Basic Info',
    fields: ['targetMajor', 'targetTerm', 'college'],
  },
...
export default function OnboardingForm({ userId }: { userId: string }) {
  ...
  const handleNext = async () => {
    const fieldsToValidate = steps[currentStep - 1]
      .fields as (keyof OnboardingFormData)[];
  ...
  return (
    <Card className="w-full max-w-2xl">
      ...
      <CardContent className="pt-4">
        <CardTitle className="text-lg font-semibold mb-6 text-center">
          ...
          {currentStep === 1 && (
            <>
              <div>
                ...
                <Input
                  ...
                  {...register('targetMajor', {
                    required: 'Target major is required',
                  })}
                  className="mt-1"
              ...
              <div>
                ...
                />
                <p className="text-sm text-gray-500 mt-1">
                  Specify the semester and year you plan to start.
                </p>
              </div>
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">
                  The name of your current or most recent institution.
                </p>
              </div>
            </>
          )}
          {currentStep === 2 && (
            <>
              <div>
                ...
                <Input
                  id="undergradMajor"
                  {...register('undergradMajor')}
                  className="mt-1"
                />
                <p className="text-sm text-gray-500 mt-1">
                  Your major during your undergraduate studies.
                </p>
              </div>
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">
                  Your Cumulative Grade Point Average. Specify scale if not 4.0
                  (e.g., 8.5/10).
                </p>
              </div>
            </>
          ...
          {currentStep === 3 && (
            <>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  ...
                  <Input
                    ...
                    className="mt-1"
                  />
                  {errors.greQuantScore && (
                    <p className="text-red-500 text-sm mt-1">
                      {getErrorMessage('greQuantScore') ||
                        'Score must be 130-170'}
                    </p>
                  )}
                </div>
                <div>
                  ...
                  {errors.greVerbalScore && (
                    <p className="text-red-500 text-sm mt-1">
                      {getErrorMessage('greVerbalScore') ||
                        'Score must be 130-170'}
                    </p>
                  )}
                </div>
              ...
              <div>
                ...
                {errors.toeflScore && (
                  <p className="text-red-500 text-sm mt-1">
                    {getErrorMessage('toeflScore') || 'Score must be 0-120'}
                  </p>
                )}
              </div>
              <div>
                ...
                <Input
                  ...
                  {...register('ielts', {
                    pattern: {
                      value: /^\d*\.?\d*$/,
                      message: 'Please enter a valid band score',
                    },
                  })}
                  className="mt-1"
                  placeholder="e.g., 7.5"
            ...
            <>
              <div>
                ...
                <Input
                  id="workExpMonths"
                  type="number"
                  {...register('workExpMonths', {
                    valueAsNumber: true,
                    min: 0,
                  })}
                  className="mt-1"
                />
                {errors.workExpMonths && (
                  <p className="text-red-500 text-sm mt-1">
                    {getErrorMessage('workExpMonths')}
                  </p>
                )}
                <p className="text-sm text-gray-500 mt-1">
                  Enter your total relevant work experience in months.
                </p>
              </div>
              <div>
                ...
                <Input
                  id="publications"
                  type="number"
                  {...register('publications', { valueAsNumber: true, min: 0 })}
                  className="mt-1"
                />
                {errors.publications && (
                  <p className="text-red-500 text-sm mt-1">
                    {getErrorMessage('publications')}
                  </p>
                )}
                <p className="text-sm text-gray-500 mt-1">
                  Enter the number of academic publications you have authored or
                  co-authored.
                </p>
              </div>
            </>
